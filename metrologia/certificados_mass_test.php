            ...
            <form method="post" enctype="multipart/form-data" id="uploadForm" name="uploadForm">
                ...
                <div class="col-md-12">
                    <div class="position-relative form-group">
                        ...
                        <input type="file" name="pdfFiles[]" class="form-control" style="width: 50%;" multiple required accept=".pdf, .zip">
                    </div>
                </div>
...
$(document).ready(function () {
    ...
    $.ajax({
        url: "templates/metrologia/includes/proceso_unique_test.php?sensor_id=<?php echo $sensor_id; ?>&mod=check",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
for (var i = 0; i < certificates.length; i++) {
    ...
    $("#dataGrid tbody").append(rowHtml);
            ...
            progress.progressTimer('error', {
                errorText: 'ERROR!',
                onFinish: function () {
                    alert('¡Hubo un error procesando su información!');
                }
            });
        } 
    });
    ...
    $("#process").click(function () {
        ...
        // Hide the "Procesar" button and show the loading progress
        $("#processButton").hide();
        $("#process").show();
        ...
        $.ajax({
            url: "templates/metrologia/includes/proceso_unique_test.php?sensor_id=<?php echo $sensor_id; ?>&mod=process",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
    ...
    $(".error-messages").removeClass("text-danger").addClass("text-success").html("¡Carga y procesamiento exitosos! Todos los certificados han sido procesados y asociados correctamente.");
     
}
            },
            error: function (xhr, status, error) {
                // Handle AJAX error
                // ... Code for handling AJAX error ...
            },
            complete: function () {
