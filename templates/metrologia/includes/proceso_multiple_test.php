...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $certificateData = array(
        'pdfFiles' => array(), // Initialize the pdfFiles array        
    );
    ...
    if ($sensor_id_from_excel === null || trim($sensor_id_from_excel) === '') {
       ...
       // continue;
            ...
            if (empty($sensor_id) || empty($sensor_id_from_excel) || empty($certificate) || empty($magnitude) || empty($issuedDate) || empty($expiresDate) || empty($status) || empty($country)) {
                $incompleteDataError++;
            }
...
foreach ($_FILES['pdfFiles']['name'] as $index => $uploadedPdfFileName) {
    $uploadedFileTmp = $_FILES['pdfFiles']['tmp_name'][$index];
    $fileExtension = pathinfo($uploadedPdfFileName, PATHINFO_EXTENSION);

    if ($fileExtension === 'pdf') {
        // Handle PDF file
        if ($uploadedPdfFileName === $expectedPdfFileName) {
            $pdfCertificateFound = true;
            // Store information about the PDF file
            $certificateData['pdfFiles'][] = array(
                'filename' => $uploadedPdfFileName,
                'tmp_path' => $uploadedFileTmp
            );
            break; // Exit the loop after finding a match
        }
    } elseif ($fileExtension === 'zip') {
        // Handle ZIP file containing PDFs
        $zip = new ZipArchive;
        if ($zip->open($uploadedFileTmp) === true) {
            for ($i = 0; $i < $zip->numFiles; $i++) {
                $zipFileName = $zip->getNameIndex($i);
                $zipFileExtension = pathinfo($zipFileName, PATHINFO_EXTENSION);
    
                if ($zipFileExtension === 'pdf' && $zipFileName === $expectedPdfFileName) {
                    $pdfCertificateFound = true;
    
                    // Construct the destination path
                    $destination = $uploadDir . '/' . $zipFileName;
    
                    // Attempt to move the PDF file directly from the ZIP archive
                    if ($zip->renameName($zipFileName, $destination)) {
                        // Store information about the moved PDF file
                        $certificateData['pdfFiles'][] = array(
                            'filename' => $zipFileName,
                            'tmp_path' => $destination
                        );
                        //echo "<br>Moved: {$zipFileName} to {$destination}";
                    } else {
                        $uploadError = true;
                        $errorMessages[] = "Error al mover el archivo PDF cargado para el certificado '{$zipFileName}'.";
                        echo "<br>Error moving: {$zipFileName}";
                    }
        ...
        $errorMessages[] = "Tipo de archivo desconocido: '$fileExtension'. Los tipos válidos son '.pdf' y '.zip'.";
    }
}
...
// Loop through the stored PDF files in $certificateData['pdfFiles'] to check for association
foreach ($certificateData['pdfFiles'] as $storedPdfFile) {
    // Assuming $storedPdfFile is the path to the stored PDF file
    $storedPdfFileName = basename($storedPdfFile);
    
    if ($storedPdfFileName === $expectedPdfFileName) {
        $pdfCertificateFound = true;        
        break;
    }
}
...
if ($mod=='process'  && $errorMessages_cnt === 0  && $unassociated === 0  && $incompleteDataError === 0 && $inconsistentDatesError === 0 && $dateFormatError === 0 && $unknownSensorTypeError === 0 && $unknownTipoError === 0) {
    ...
    foreach ($certificates as $certificate) {
       ...
       // echo "<pre>"; print_r($certificateData); echo "</pre>";  
       ...
       // $res = $db_cms->insert_query_last($insertQuery);
        ...
        if ($res == FALSE) {
            ...
            foreach ($certificateData['pdfFiles'] as $index => $pdfFileData) {
                if ($pdfFileData['filename'] === $expectedPdfFileName) { // Use 'filename' key
                    $pdfFileTmp = $pdfFileData['tmp_path']; // Use 'tmp_path' key
                    break;
                }
            }
...
if ($tipo === 'Primario') {
  ...
  echo "<br>".$pdfFileTmp."===".$destination; 
   
  echo "<br>".$movefile = move_uploaded_file($pdfFileTmp, $destination);
